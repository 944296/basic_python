#리스트 List 데이터 타입
#여러가지 종류의 데이터를 담는 자료혈
#형태 : [데이터, 데이터2,...., 데이터N]
#요소/원소 : 리스트안에 들어있는 데이터
#----------------------------------
#리스트 객체 생성
emList=[]
numList=[5,23,0.9,7,-10]
strList=["A","bcd","한글","123"]
dataList=[12,"A",True,235]
dataList2=[1,2,3,["a","b",[]]]

#리스트 안에 요소 수 출력 => 내장함수 len(변수명)
print(f"emList의 갯수 : {len(emList)}개, {emList}")
print(f"numList의 갯수 : {len(numList)}개, {numList}")
print(f"strList의 갯수 : {len(strList)}개, {strList}")
print(f"dataList의 갯수 : {len(dataList)}개, {dataList}")
print(f"dataList2의 갯수 : {len(dataList2)}개, {dataList2}")
...
#슬라이싱
#[시작인덱스:끝인덱스+1] => 범위 시작인덱스 이상 <= ~ < 끝인덱스 미만

#1번 요소~ 3번 요소
print(nums[1:4])
#0번 요소부터 짝수 인덱스 요소만 출력 => [시작인덱스:끝인덱스+1:간격]
print(nums[::2])
