...
#pop()/POP(인덱스): 데이터를 꺼내는 메서드
#                 : 리스트에서 데이터 제거
nums=[1,2,3,"ABC",False,3]
#데이터 3, "ABC"를 삭제해 주세요.
# => remove(), pop()
#방법1) list.remove(데이터)메서드
#       해당 데이터가 리스트에 있다면 삭제
#       해당 데이터가 여러개 있는 경우 제일 첫번째것 제거
print(f"nums => {nums}, 요소수 : {len(nums)} 개")
#nums=[1,2,3,"ABC",False,3]
#      0 1 2   3   4     5
nums.remove(3)
print(f"nums => {nums}, 요소수 : {len(nums)} 개")
#nums=[1,2,"ABC",False,3]
#      0 1  2  3    4    
nums.remove(3)
print(f"nums => {nums}, 요소수 : {len(nums)} 개")
print(nums[2])

#방법2) list.pop() : 제일 마지막 요소 꺼내기
#       list.pop(인덱스): 지정된 인덱스의 데이터 꺼내기/삭제
