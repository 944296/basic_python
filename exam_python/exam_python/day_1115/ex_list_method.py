...
# List.index(요소) => 없으면 Error/ 있으면 인덱스
nIdx=nums.index(5)
print(f"5의 인덱스 :{nIdx}")
...
#List.pop(index)    :지정된 인덱스의 요소를 꺼내기
picknum=nums.pop()
...
#5)리스트의 요소/원소 추가하는 메서드-(1)
#List.append() : 제일 마지막에 추가
print(f"nums => {nums}, {len(nums)}개")
#nums[0] = 10 # <=인덱스란? 이미 존재하는 데이터에 부여된 번호
nums.append(3)
nums.append(1)
nums.append(8)
print(f"nums => {nums}, {len(nums)}개")

#5)리스트의 요소/원소를 지정된 위치에 추가하는 메서드-(2)
...
nums.insert(len(nums),11)  # 0 1 2 3번째 자리에 11을 넣어줘
print(f"nums => {nums}, {len(nums)}개") # 1 2 3개

#7)리스트 요소/원소 정렬하는 메서드
#List.sort()
#기준: 작은값 >>>>>  큰값  오름차순(ascending) 정렬
#      큰값  >>>>>> 작은값 내림차순(descending) 정렬
nums.sort()
print(f"nums => {nums}, {len(nums)}개")
